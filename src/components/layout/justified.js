...
const getCommonWidgth = (columns, containerWidth, margin) => {
  const rowWidth = (containerWidth/columns)- ((columns * 5) + (columns * 2 ));
  return rowWidth;
};
...
export const computeRowLayout = ({ containerWidth, limitNodeSearch, targetRowHeight, margin, photos,columns }) => {
  // const t = +new Date();
  ...
  path = path.map(node => +node);
   const width   = getCommonWidgth(columns, containerWidth, margin);
